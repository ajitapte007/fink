"""SQLite read-through cache: metadata, corruption marking, stale fallback.

Ported from mcp/tests in the phase 3 fork. The imports now span two modules
rather than one, which is itself worth exercising: `get_av_data` lives in the
vendor package and the SQLite primitives in `cache`, and the split is only
correct if they still cooperate.
"""
import time
import sqlite3

from mcp_apps.data.alphavantage.fetch import get_av_data
from mcp_apps.data.cache import get_db_path, init_db
...
def test_cache_corruptness_and_sync_refetch():
    from mcp_apps.data.alphavantage.fetch import (get_all_data_for_ticker,
                                                   is_ticker_cache_corrupt)
    from mcp_apps.data.cache import get_db_cache, set_db_cache
    import sqlite3
    ...
    import mcp_apps.data.alphavantage.fetch as fetch_utils
