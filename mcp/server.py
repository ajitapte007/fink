...
def _resolve_end_year(end_year: Optional[int]) -> Optional[int]:
    """Default an unset end_year to the current year.

    Note this only fills an *omitted* end_year. If the model asserts a wrong year
    outright, that value is respected — a user may legitimately want a window that
    ends in the past.
    """
    if end_year is None:
...
def visualize_html(
    ...
    result = cache_ticker_data(ticker)
    if result["error"]:
