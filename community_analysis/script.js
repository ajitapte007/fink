...
let loadedTabs = { raw: false, recipes: false, ai: false, debate: false };
let lastTicker = '';
let lastApiKey = '';

function resetLoadedTabs() {
    loadedTabs = { raw: false, recipes: false, ai: false, debate: false };
    aiReportContainer.style.display = 'none';
    aiEmptyState.style.display = 'block';
    
    const chatContainer = document.getElementById('debateChatContainer');
    const dbEmptyState = document.getElementById('debateEmptyState');
...
function fetchTabData(tab) {
    ...
    if (tab === 'raw' && !loadedTabs.raw) {
        ...
        loadedTabs.raw = true;
    } else if (tab === 'ai' && !loadedTabs.ai) {
        // AI tab handles its own fetching via the "Run Full Analysis" button 
        // to prevent accidental expensive LLM calls
        loadedTabs.ai = true; 
    } else if (tab === 'debate' && !loadedTabs.debate) {
        loadedTabs.debate = true;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    async function runAiAnalysis() {
        const ticker = getCurrentTicker();
        if (!ticker) {
            showErrorModal("Please enter a ticker symbol first.");
            return;
        }
        ...
        try {
            const response = await fetch('http://127.0.0.1:8000/analyze-stock', {
                method: 'POST',
                headers: {
            ...
            showErrorModal(`AI Analysis Failed:\n\n${error.message}`);
            aiEmptyState.style.display = 'block';
        } finally {
    ...
    function renderAiReport(report) {
        let html = `
            <!-- Executive Summary -->
            <div class="mb-8">
                <h3 class="text-xl font-bold text-gray-800 mb-3 border-b pb-2">🎯 Executive Summary</h3>
                <p class="text-gray-700 leading-relaxed">${report.executive_summary}</p>
                <div class="mt-4 p-4 bg-gray-50 rounded-lg border text-gray-700">
                    <p><strong>Moderator Commentary:</strong> ${report.moderator_commentary}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="bg-blue-50/50 p-5 rounded-xl border border-blue-100">
                    <h4 class="font-bold text-blue-900 mb-3 flex items-center gap-2">🚀 Key Catalysts</h4>
                    <ul class="list-disc pl-5 text-sm text-blue-800 space-y-1">
                        ${report.key_catalysts.map(c => `<li>${c}</li>`).join('')}
                    </ul>
                </div>
                <div class="bg-red-50/50 p-5 rounded-xl border border-red-100">
                    <h4 class="font-bold text-red-900 mb-3 flex items-center gap-2">⚠️ Primary Risks</h4>
                    <ul class="list-disc pl-5 text-sm text-red-800 space-y-1">
                        ${report.primary_risks.map(r => `<li>${r}</li>`).join('')}
                    </ul>
                </div>
            </div>
            ...
            <div class="mb-8">
                <h3 class="text-xl font-bold text-gray-800 mb-3 border-b pb-2">⚔️ Committee Cross-Examination</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ${report.cross_examination_results.map(exam => `
                        <div class="p-4 border rounded-lg text-sm bg-white shadow-sm hover:shadow-md transition-shadow">
                            <div class="font-semibold text-gray-700 mb-2 pb-2 border-b">
                                ${exam.reviewer} <span class="text-gray-400 font-normal mx-1">critiques</span> ${exam.target}
                            </div>
                            ${exam.critiques.length > 0 ? `
                                <div class="mb-2">
                                    <span class="text-xs font-bold text-red-600 uppercase">Critique</span>
                                    <p class="text-gray-600 mt-1 line-clamp-3">${exam.critiques[0]}</p>
                                </div>
                            ` : ''}
                            ${exam.bias_flags.length > 0 ? `
                                <div>
                                    <span class="text-xs font-bold text-orange-500 uppercase">Bias Flag</span>
                                    <p class="text-gray-600 mt-1 line-clamp-2">${exam.bias_flags[0]}</p>
                                </div>
                            ` : ''}
                        </div>
                    `).join('')}
            ...
            <!-- Agent Theses Grid -->
            <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-2">📊 Analyst Theses</h3>
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ${renderAgentCard(report.bull_case, 'text-green-700', 'bg-green-50')}
                ${renderAgentCard(report.neutral_case, 'text-gray-700', 'bg-gray-50')}
                ${renderAgentCard(report.bear_case, 'text-red-700', 'bg-red-50')}
            </div>

            <!-- Disclaimer -->
            <div class="text-center text-xs text-gray-400 mt-12 mb-4">
                ${report.disclaimer}
            </div>
        `;

        aiReportContainer.innerHTML = html;
        aiReportContainer.style.display = 'block';
    }

    function renderAgentCard(agentData, textClass, bgClass) {
        return `
            <div class="border rounded-xl p-5 ${bgClass} shadow-sm">
                <div class="flex justify-between items-center mb-3">
                    <h4 class="font-bold ${textClass} text-lg">${agentData.agent}</h4>
                    <span class="text-xs font-semibold px-2 py-1 bg-white rounded-full border">
                        Conf: ${Math.round(agentData.confidence * 100)}%
                    </span>
                </div>
                <p class="text-sm font-medium text-gray-800 mb-4 italic">"${agentData.thesis}"</p>
                
                <div class="mb-3">
                    <h5 class="text-xs font-bold text-gray-500 uppercase mb-1">Key Points</h5>
                    <ul class="list-disc pl-4 text-xs text-gray-700 space-y-1">
                        ${agentData.key_points.map(p => `<li>${p}</li>`).join('')}
                    </ul>
                </div>
                
                <div class="mb-3">
                    <h5 class="text-xs font-bold text-gray-500 uppercase mb-1">Data References</h5>
                    <div class="flex flex-wrap gap-1">
                        ${agentData.data_references.slice(0, 4).map(ref => `
                            <span class="text-[10px] bg-white border px-1.5 py-0.5 rounded text-gray-600 truncate max-w-full">${ref}</span>
                        `).join('')}
                        ${agentData.data_references.length > 4 ? `<span class="text-[10px] text-gray-500 ml-1">+${agentData.data_references.length - 4} more</span>` : ''}
                    </div>
                </div>
            </div>
        `;
...
const debateLoadingSpinner = document.getElementById('debateLoadingSpinner');
const debateChatContainer = document.getElementById('debateChatContainer');
const debateEmptyState = document.getElementById('debateEmptyState');
...
if (startDebateBtn) {
    startDebateBtn.addEventListener('click', async () => {
        ...
        startDebateBtn.classList.add('opacity-50', 'cursor-not-allowed');
        debateLoadingSpinner.style.display = 'inline-block';
        debateEmptyState.style.display = 'none';
        debateChatContainer.style.display = 'flex';
        debateChatContainer.innerHTML = '';

        try {
            const response = await fetch('http://127.0.0.1:8000/live-debate/stream', {
                method: 'POST',
            ...
            console.error(error);
            showErrorModal(`Live debate failed: ${error.message}`);
        } finally {
            startDebateBtn.disabled = false;
            startDebateBtn.classList.remove('opacity-50', 'cursor-not-allowed');
            debateLoadingSpinner.style.display = 'none';
        }
    });
}

function renderDebateMessage(msg) {
    const msgDiv = document.createElement('div');
    msgDiv.className = 'p-4 rounded-lg shadow-sm border bg-white animate-fade-in';
    
    // Style based on speaker
    let nameColor = "text-gray-800";
    let icon = "🗣️";
    
    if (msg.speaker.includes("Bull")) {
        nameColor = "text-green-600";
        icon = "🚀";
        msgDiv.classList.add('border-green-100');
    } else if (msg.speaker.includes("Bear")) {
        nameColor = "text-red-600";
        icon = "🐻";
        msgDiv.classList.add('border-red-100');
    } else if (msg.speaker.includes("Neutral")) {
        nameColor = "text-blue-600";
        icon = "⚖️";
        msgDiv.classList.add('border-blue-100');
    } else if (msg.speaker === "System") {
        nameColor = "text-purple-600";
        icon = "⚙️";
        msgDiv.className = 'p-3 text-center text-sm font-medium text-gray-500 bg-gray-100 rounded-full my-2 animate-fade-in mx-auto px-6 border';
    }

    if (msg.speaker !== "System") {
        let refsHtml = '';
        if (msg.references && msg.references.length > 0) {
            refsHtml = `<div class="mt-2 pt-2 border-t border-gray-100 text-xs text-gray-500">
                <span class="font-medium">Replying to:</span> ${msg.references.join(', ')}
            </div>`;
        }

        msgDiv.innerHTML = `
            <div class="flex justify-between items-center mb-2">
                <div class="font-bold ${nameColor} flex items-center gap-2">
                    <span>${icon}</span> ${msg.speaker}
                </div>
        ...
        `;
         ...
         msgDiv.innerHTML = `<span>${icon}</span> ${msg.message}`;
    }

    debateChatContainer.appendChild(msgDiv);
